<!-- Main Content-->
<div class="main-content pt-0">
	<div class="side-app">
		<div class="main-container container-fluid">
			<!-- Page Header -->
			<div class="page-header" style="min-height:unset;">
				<h4 class="text-start mb-0"><?php echo "" . ' ' . UCWORDS(str_replace('-', ' ', $this->page_name)); ?></h4>
				<!--div class="btn btn-list">
					<a href="<?php echo BASE_URL . 'dispatch-list'; ?>">
					<button aria-label="Close" class="btn ripple btn-dark" type="button"><span aria-hidden="true">&times;</span></button></a>
				</div-->
			</div>
			<!-- End Page Header -->
			<!-- Row -->
				<div class="row">
					<div class="col-lg-12">
						<div class="card custom-card">
							<div class="card-body">
								<?php $this->load->view('Backend/Elements/Alert.php'); ?>
								<div class="row">
									<div class="col-md-3">
										<div class="row">
											<div class="col-md-12">
												<div class="form-group">
													<label class="form-label">Select Location: <span class="tx-danger">*</span></label>
													<select class="form-control select2" name="stock_audit_location_no" required id="stock_audit_location_no">
														<option value="">Select customer</option>
														<?php echo $this->occupied_location_option; ?>
													</select>
												</div>
											</div>

											<div class="col-md-12 hid-tbl">
												<label class="form-label">Scan Box: <span class="tx-danger">*</span></label>
												<input class="form-control" name="stock_audit_box_no" id="stock_audit_box_no" value="" type="text">
												<span id="error_msg" class="tx-danger"></span>
											</div>
										</div>
									</div>
									<div class="col-md-9">
										<div class="table-responsive hid-tbl">
											<h5>Available List</h5>
											<table class="table table-striped table-bordered text-wrap" style="border-left:1px solid #e1e6f1;">
												<thead>
													<tr>
														<th>Sl. No.</th>
														<th>Material Type</th>
														<th>Item Name</th>
														<th>Total Qty</th>
														<th>Remaining Qty</th>
														<th>Box No.</th>
													</tr>
												</thead>
												<tbody id="prod_list">

												</tbody>
											</table>
										</div>

										<div class="table-responsive verified-list-tbl  hid-tbl">
											<h5>Verified List</h5>
											<table class="table table-striped table-bordered text-wrap" style="border-left:1px solid #e1e6f1;">
												<thead>
													<tr>
														<th>Sl. No.</th>
														<th>Box No.</th>
														<th>Location No.</th>
														<th>Location Name</th>
														<th>Audit Date</th>
													</tr>
												</thead>
												<tbody id="Verified_list">
												</tbody>
											</table>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
				<!-- End Row -->
		</div>
	</div>
</div>
<!-- End Main Content-->